    ...
    <div class="container-fluid flex-grow-1 container-p-y">
        <div class="row">
            <div class="col-md-6 text-start">
                <h5 class="py-2 mb-2">
                    <span class="text-primary fw-light">Stock Out Management</span>
                </h5>
            </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            const table = $('#itemTable').DataTable({
                ...
                ajax: {
                    url: "{{ route('admin.stockOut.getall') }}",
                    type: 'GET',
                },
                columns: [
            ...
            $("#saveStockIn").on("click", function(e) {
                ...
                $.ajax({
                    url: "{{ route('admin.stockOut.store') }}",
                    method: "POST",
                    data: formData,
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            setFlash("error", response.message ??
                                "Something went wrong while saving Stock Out.");
                        }
                    },
                    error: function(xhr) {
                        setFlash("error", xhr.responseText ??
                            "Server error while saving Stock Out.");
                    }
                });
            });
            ...
            window.updateStockStatus = function(stockId, status) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            type: "POST",
                            url: "{{ route('admin.stockOut.status') }}",
            ...
            window.deleteStock = function(userId) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        const url = '{{ route("admin.stockOut.destroy", ":userId") }}'.replace(":userId", userId);
                        $.ajax({
                            type: "DELETE",
                            url,
